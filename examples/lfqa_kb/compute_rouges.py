...
def rouge(ref_data, prediction_file):
    ...
    for i, row in baseline_df.iterrows():
        hf_rouge_score, kilt_rouge_score = _metric_max_over_ground_truths(row["text"],ref_data[row["id"]]['output']) # prediction_text
...
import glob
pred_files = glob.glob("/dccstor/srosent2/generative/baseline_llms/ELI5-full/512-limit/*/*/*/predictions*")
