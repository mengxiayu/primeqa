...
from primeqa.lfqa_kb.processors.preprocessors.fid_kg import preprocess_eli5_function_fid, preprocess_eli5_validation_function_fid
from primeqa.lfqa_kb.processors.postprocessors.eli5 import postprocess_eli5_function
from primeqa.lfqa_kb.models.moe_bart import MoEBART
...
class DataTrainingArguments:
    ...
    dataset_name: Optional[str] = field(# Post-processing:
...
def main():
    ...
    parser = HfArgumentParser((ModelArguments, LFQADataArguments, LFQATrainingArguments))
    ...
    #         use_auth_token=True if model_args.use_auth_token else None,
    #     )
    #     model = MoEBART(bart.config)
    #     # model.load_pretrained(bart.state_dict())
    # else:
    model = MoEBART.from_pretrained(
        model_args.model_name_or_path,
        from_tf=bool(".ckpt" in model_args.model_name_or_path),
        config=config,
        cache_dir=model_args.cache_dir,
    ...
    if training_args.do_eval:
        ...
        with training_args.main_process_first(desc="validation dataset map pre-processing"):
            eval_dataset = eval_examples.map(
                preprocess_eli5_validation_function_fid,
                fn_kwargs=args_for_preprocess,
                batched=True,
                num_proc=data_args.preprocessing_num_workers,
                remove_columns=column_names,
    ...
    trainer = QuestionAnsweringSeq2SeqTrainer(
        ...
        eval_dataset=eval_dataset if training_args.do_eval else None, # processed dataset format
        eval_examples=eval_examples if training_args.do_eval else None, # raw dataset format
        data_args=data_args,
        tokenizer=tokenizer,
